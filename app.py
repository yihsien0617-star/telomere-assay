...
model_basis = st.sidebar.radio(
    ...
    help="針對用於建立模型的資料集，推薦只取每人的「首次」數據，以排除後續干預影響。"
)
...
if uploaded_file is not None:
    ...
    if err:
        st.error(f"分析資料讀取錯誤: {err}")
        st.stop()
        
    # 2. 決定模型使用的資料來源 (Ref DF)
    if uploaded_ref_file is not None:
        df_ref, err_ref = load_data(uploaded_ref_file)
        if err_ref:
            st.warning(f"常模資料讀取錯誤: {err_ref}，將降級使用主分析資料建立模型。")
            df_ref = df.copy()
            source_name = "主分析資料 (無額外常模)"
        else:
            source_name = "獨立常模資料庫"
    else:
        df_ref = df.copy()
    ...
    if len(valid_train) > 5:
        ...
        # 顯示模型資訊
        st.sidebar.success(f"✅ 模型已建立\n- 來源: {source_name}\n- 樣本數: {len(valid_train)} 人")
        st.sidebar.markdown(f"`y = {slope:.4f}x + {intercept:.2f}`")
        ...
        # 也幫 Ref 資料算一下 (為了 Tab 3 畫圖用)
        df_ref['端粒年齡'] = df_ref['端粒長度'].apply(calculate_telomere_age)
        
    else:
        st.warning("⚠️ 有效樣本數不足，無法建立模型。")
        slope, intercept = 0, 0
        df['端粒年齡'] = df['年紀']
    ...
    for name, group in df.groupby('姓名'):
        if len(group) >= 2:
            ...
            person_stats.append({
                ...
                '監測天數': (last_rec['篩檢日期'] - first_rec['篩檢日期']).days,
                '首次日期': first_rec['篩檢日期'],
                '末次日期': last_rec['篩檢日期']
            })
    ...
    with tab2:
        ...
        sel_team_ind = st.selectbox("1. 選擇所屬團隊", all_teams, key="ind_team")
        people_in_team = sorted(df[df['備註'] == sel_team_ind]['姓名'].unique())
        sel_people = st.multiselect("2. 勾選人員 (可多選)", people_in_team, default=people_in_team[:1] if people_in_team else None)
    ...
    # === Tab 3: 常模分析 (更新版) ===
    with tab3:
        st.subheader("年齡 vs. 端粒長度 (常模落點分析)")
        st.markdown(f"**目前模型來源**：{source_name} (共 {len(valid_train)} 筆參考數據)")
        
        # 繪圖邏輯：
        # 1. 先畫「背景常模」(灰色點)：如果是獨立檔案，這些就是背景值。
        # 2. 再畫「主分析資料」(彩色點)：這是您關注的對象。
        # 3. 疊加「常模趨勢線」(紅虛線)。

        fig_scatter = go.Figure()

        # 1. 背景常模點 (如果來源不同，或者是全部資料)
        # 為了避免圖太亂，如果來源相同，就不用畫兩次背景
        if source_name == "獨立常模資料庫":
            fig_scatter.add_trace(go.Scatter(
                x=df_ref['年紀'], y=df_ref['端粒長度'],
                mode='markers', name='常模資料庫',
                marker=dict(color='lightgray', size=5, opacity=0.5),
                hoverinfo='none' # 背景資料不顯示詳細資訊，避免干擾
            ))

        # 2. 主分析資料點 (可篩選團隊)
        sel_corr_teams = st.multiselect("篩選顯示團隊 (分析對象)", all_teams, default=all_teams, key="corr_team")
        if sel_corr_teams:
            df_target = df[df['備註'].isin(sel_corr_teams)]
            for team_name in sel_corr_teams:
                team_data = df_target[df_target['備註'] == team_name]
                fig_scatter.add_trace(go.Scatter(
                    x=team_data['年紀'], y=team_data['端粒長度'],
                    mode='markers', name=team_name,
                    text=team_data['姓名'],
                    hovertemplate="<b>%{text}</b><br>年紀: %{x}<br>長度: %{y:.3f}<br>端粒年齡: %{customdata:.1f}",
                    customdata=team_data['端粒年齡']
                ))
        ...
        fig_scatter.add_trace(go.Scatter(
            x=x_range, y=y_pred, mode='lines', name='標準趨勢線',
            line=dict(color='red', dash='dash', width=2)
        ))

        fig_scatter.update_layout(
            xaxis_title="實際年齡", yaxis_title="端粒長度",
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
        )
        st.plotly_chart(fig_scatter, use_container_width=True)
        st.info("💡 說明：灰色點(若有)為背景資料庫，彩色點為本次分析對象。紅色虛線代表該年齡層的平均水準。")
    ...
    with tab4:
        ...
        if not team_changes.empty:
            ...
            c1.metric("平均首次", f"{team_changes['首次數值'].mean():.3f}")
            c2.metric("平均末次", f"{team_changes['末次數值'].mean():.3f}")
